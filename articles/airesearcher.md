...
Rust を選んだ理由は一つです。**プロトタイプと本番が同じコードになる**から。

Python で研究し、Rust で書き直して本番に乗せる——これが業界の標準フローです。研究者が実装した Python コードを、エンジニアが Rust や C++ に翻訳する。ソロ開発者にとって、この「翻訳コスト」は致命的です。同じロジックを二言語で実装するか、品質を妥協するか、二択を迫られる。Rust ならその二択が消えます。`impl Trait` のゼロコスト抽象化で研究段階の試行錯誤をそのまま本番に持っていける。抽象化のオーバーヘッドはコンパイル時に消え、実行時コストはゼロです。

オンデバイスで動かす以上、**GC によるポーズは許容できません**。会話の途中で数百ms の GC ポーズが入れば、製品として死んでいます。Rust は GC を持たない。メモリ安全性は実行時ではなくコンパイル時の所有権システムで保証するので、ホットパスにランタイムコストが入りません。

型システムも研究のイテレーション速度を上げます。Python の DL フレームワークはテンソルの形状不整合を実行時にしか検出できません。Rust なら `cargo check` でコンパイル前に洗い出せる。「動かしてみたらシェイプエラー」に費やす時間が消えます。

**クレート構成:**

- **DL**: `candle-core` + `candle-nn`（研究・推論）、`Burn`（本番トレーニング）
- **GPU**: `CubeCL`——Burn の Rust-native GPU カーネル言語。CUDA / Metal / Vulkan を同一の Rust 構文で記述し、バックエンド切り替えがコード変更ゼロ
